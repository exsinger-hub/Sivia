...
const tools = [
  ...
  },
  {
    ...
    description: "Read the available draw.io graph primitives, registered shapes, edge styles, markers, and the Sivia MCP coverage. This never launches draw.io and never changes the canvas; when already connected it also inspects the live renderer registry.",
    inputSchema: {
      type: "object",
      properties: {
...
async function getCapabilities(args = {}) {
  ...
  return {
    detection: {
      ...
      basis: connected ? ["Sivia tool catalog", "live draw.io renderer registry"] : ["Sivia tool catalog"],
    },
...
async function addShape(args) {
  ...
  if ((!renderability.renderer_registered && !renderability.stencil_registered) || renderability.unknown_bare_tokens.length) {
    ...
    throw new Error(`The draw.io style for "${requestedShape}" is not renderable (${detail}). Sivia refused the renderer's silent rectangle fallback. Use a baseline or registered capability, reconstruct the object from editable primitives, or insert only the smallest irreducible raster region.`);
  }
...
async function updateTableLayout(args) {
  ...
  const value = await graphEval(`
    ...
    if (semanticType !== 'table') throw new Error('Cell is not a Sivia editable table: ' + a.table_id);
