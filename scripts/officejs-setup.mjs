...
async function generateCertificate() {
  ...
  const common = [
    ...
    "-subj", "/CN=localhost/O=Sivia Local Development",
  ];
  try {
    ...
    const config = [
      ...
      "[req_dn]", "CN=localhost", "O=Sivia Local Development",
...
function nextSteps() {
  if (process.platform === "darwin") {
    return [
      ...
      "In a new Codex task, select Sivia and call powerpoint_officejs_status once to start the local bridge.",
      "Then in PowerPoint, open Insert > My Add-ins > Sivia Live and keep the task pane open.",
  ...
  return [
    ...
    "Call powerpoint_officejs_status in a new Codex task to start the bridge, then open Sivia Live in PowerPoint and check status again.",
  ];
}
