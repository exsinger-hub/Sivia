...
check({r['id'] for r in active}=={'neuralangelo','react','diffdock'},'reference pool differs from user acceptance')
check(idx['total_pairs']==len(active)==3,'active count')
check(idx['publication_allowed'] is False,'publication permission must remain false')
...
check(idx['new_approved_pairs']==0,'new approval count')
...
check(db.execute('SELECT count(*) FROM matches').fetchone()[0]==72,'SQLite matching rows')
...
db.close()
report={'valid':not errors,'checks':checks,'errors':errors,'active_legacy':3,'excluded_history':39,'candidates':24,'actual_drafts':3,'new_admitted':0,'publication_allowed':False,'scope':'structural/provenance validation only, not visual acceptance'}
