...
When the result violates the already specified content or output requirements, repair the smallest failed region within the authorized scope and inspect it again; discovering a defect is not completion. Apply [Asset Production's failure-to-action loop](asset-production.md#turn-a-failed-attempt-into-a-different-next-action) when the cause is source resolution, background, decomposition or final-size readability. Keep full revised figure prompts above their bound length floor. A repair may not invent missing data, expand the approved layout or bypass the user's image/PPT choice. If a required source or user decision is unavailable, show the limitation rather than certifying the draft.
...
Read [Asset Production](asset-production.md) completely before choosing reconstruction assets. It governs objects with no built-in icon, actual SVG editability, standalone illustration generation and retained-pixel checks. Use the same decisions during image-only asset planning without starting native construction.
